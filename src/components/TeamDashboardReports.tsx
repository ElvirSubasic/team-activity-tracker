    ...
    ? `conic-gradient(${contributionSegments
        .map((row, index) => {
          const start = contributionSegments
            .slice(0, index)
            .reduce((sum, entry) => sum + entry.contribution_percent, 0);
          const end = start + row.contribution_percent;
  ...
  return (
    <section className="panel">
      <div className="panel-header">
        <div>
          <h2>Team Dashboards & Reports</h2>
          <p className="hint">Review team performance metrics and share leaderboard updates.</p>
        </div>
      </div>
        ...
        <div className="form-section">
          ...
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              void handleExport(
                () => window.desktop.report.exportPersonLogsCsv(selectedPersonId, filters),
                "Per-person logs"
              );
            }}
          >
